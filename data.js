...
/* ── 헬퍼 등급(티어) ──────────────────────────────────────────
   누적 수행건수·평점·분쟁률로 결정되는 규칙 기반 배지(무작위 아님).
   업계 표준(TaskRabbit Elite·우버 다이아 등)의 '검증된 상위 공급자' 신호.
   min* 조건을 모두 만족하는 가장 높은 등급이 부여된다.                     */
const HELPER_TIERS = [
  { id: 'new',      label: '신규',      icon: '🌱', color: '#8b6f47', minJobs: 0,   minRating: 0,    maxDispute: 1,    desc: '활동 시작 단계 · 콜드스타트 노출 보장' },
  { id: 'verified', label: '인증',      icon: '✓',  color: '#c5a46e', minJobs: 50,  minRating: 0,    maxDispute: 0.15, desc: '신원·자격 검증 완료, 안정적으로 활동 중' },
  { id: 'elite',    label: '엘리트',    icon: '★',  color: '#d9b451', minJobs: 100, minRating: 4.8,  maxDispute: 0.08, desc: '높은 평점과 수행량을 꾸준히 유지' },
  { id: 'elitepro', label: '엘리트 프로', icon: '👑', color: '#e5b95a', minJobs: 200, minRating: 4.9,  maxDispute: 0.05, desc: '상위 1% · 최우선 매칭 노출 · 분쟁 최소' },
];
function findTier(id) { return HELPER_TIERS.find(t => t.id === id) || HELPER_TIERS[0]; }

/* ── 후기 문장 코퍼스 ─────────────────────────────────────────
   태그별 문장 풀 + 별점대별 마감 문장. 후기 텍스트가 표시된 칭찬 태그와
   의미상 일치하도록 태그를 키로 둔다(문맥 불일치 방지).                    */
const REVIEW_CORPUS = {
  ontime:  ['정확히 약속 시간에 도착했어요.', '1분도 안 늦고 딱 맞춰 오셨습니다.', '시간 약속을 칼같이 지켜주셔서 좋았어요.'],
  kind:    ['시종일관 친절하셨어요.', '인사도 밝고 매너가 정말 좋으셨습니다.', '기분 좋게 응대해 주셔서 감사했어요.'],
  careful: ['꼼꼼하게 마무리해 주셨어요.', '깔끔하게 정리까지 해주셨습니다.', '세심하게 신경 써주신 게 느껴졌어요.'],
  comm:    ['진행 상황을 계속 알려주셔서 안심됐어요.', '연락이 빨라서 소통이 편했습니다.', '중간중간 사진으로 확인시켜 주셨어요.'],
  gentle:  ['물건을 조심스럽게 다뤄주셨어요.', '파손 하나 없이 옮겨주셨습니다.', '소중한 물건인데 정말 조심히 해주셨어요.'],
  pro:     ['역시 전문가답게 처리하시네요.', '노하우가 느껴지는 깔끔한 작업이었어요.', '군더더기 없이 프로답게 끝내주셨습니다.'],
};
/* 별점 4점대: 만족하되 사소한 아쉬움 / 3점대: 무난하나 아쉬움 (정직한 분포용) */
const REVIEW_MILD_4 = ['전반적으로 만족합니다. 조금만 더 빨랐으면 좋았을 것 같아요.', '무난하게 잘 해주셨어요. 다음에도 부탁드릴게요.', '결과는 좋았고 응대도 괜찮았습니다.'];
const REVIEW_MILD_3 = ['시간이 조금 걸렸지만 마무리는 괜찮았어요.', '소통이 조금 아쉬웠지만 결과는 나쁘지 않았습니다.', '기대보단 평범했지만 무난했어요.'];
/* 마스킹된 요청자 표기(개인정보 최소 — 실제 서비스의 익명 후기 표기 방식) */
const REVIEWER_INITIALS = ['김', '이', '박', '최', '정', '강', '조', '윤', '장', '임', '한', '오', '서', '신', '권'];
